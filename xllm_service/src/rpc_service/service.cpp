...
bool XllmRpcServiceImpl::handle_generation(const llm::RequestOutput& request_output) {
  ...
  OutputCallback cb;
  {
    ...
    if (it == callbacks_.end()) {
      LOG(ERROR) << "Can not found the callback for the received request output, request id is: " << service_request_id;
      return false;
    }
    cb = it->second;
  }
  ...
  size_t req_thread_idx = -1;
  {
    ...
    if (it == remote_requests_output_thread_map_.end()) {
      LOG(ERROR) << "Can not found the thread for the received request output, request id is: " << service_request_id;
      return false;
    }
  ...
  output_threadpools_[req_thread_idx].schedule(
      [this, service_request_id, cb, request_output = std::move(request_output)]() mutable {
...
bool XllmRpcServiceImpl::record_new_request(std::shared_ptr<ChatCallData> call_data,
                                            ...
                                            bool include_usage) {
  {
    ...
    if (callbacks_.find(service_request_id) != callbacks_.end()) {
      LOG(ERROR) << "The request ID already exists. Requests with the same ID are not allowed. " << service_request_id;
      return false;
    }
    callbacks_[service_request_id] = [this,
                                      call_data,
                                      model,
                                      stream,
                                      include_usage,
...
bool XllmRpcServiceImpl::record_new_request(std::shared_ptr<CompletionCallData> call_data,
                                            const std::string& service_request_id,
                                            bool stream,
                                            const std::string& model,
                                            bool include_usage) {
...
void XllmRpcService::Generations(
    google::protobuf::RpcController* cntl_base,
  ...
  for (auto& request : req->gens()) {
    ...
    if (request.has_gen_status()) {
      request_output.status =
          llm::Status(static_cast<llm::StatusCode>(request.gen_status().status_code()),
                 request.gen_status().status_msg());
    }
    if (request.has_usage()) {
      llm::Usage u;
      u.num_prompt_tokens =
          request.usage().num_prompt_tokens();
      u.num_generated_tokens =
          request.usage().num_generated_tokens();
      u.num_total_tokens =
          request.usage().num_total_tokens();
    ...
    for (auto& output : request.outputs()) {
      ...
      sequence_output.token_ids =
          std::vector<int32_t>(output.token_ids().begin(), output.token_ids().end());
